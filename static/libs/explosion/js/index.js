...
class ExplosionGallery {
    constructor(elementNode, options) {
        ...
        this.minHeight = 600;
        this.padding = 2 * 16;
        this.showingCount = 4; // how much photo will be shown
        this.currentIndex = 0;
    ...
    closeGallery = () => {
        ...
        this.explosionCloseNode.style.marginTop = '3000px';
    ...
    switchImage = (event) => {
        ...
        if( buttonNode.classList.contains(explosionNavNextClassName) &&
    ...
    activateGallery = (event) => {
        ...
        if( !linkNode ||
            ...
            this.modalContainerNode.classList.contains(explosionOpeningClassName) ) { // if dont click on image
            return;
        }
    ...
    setCurrentState() {
        // 5 arrays with photos in dif pos
        ...
        this.explosionImageNodes.forEach((imageNode, index) => {
            ...
            } else if( this.currentIndex + this.showingCount >= index ) {
    ...
    setGalleryStyles() {
        ...
        const modalWigth = Math.max(this.minWidth, window.innerWidth)
        ...
        this.setControlsStyles(this.explosionControlsNode, {
            marginTop: (modalHeight - modalHeight * 1.2) / 2,
...
/**
 * Helpers
 * change opacity from 0 to 1 animation-> then call callback
...
/**
 * исчезание модального окна с помощью изменения opacity
 */
...
/**
 * оптимизация при уменьшения окна
 */
